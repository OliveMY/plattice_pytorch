...
void createHashTable(int capacity) {
    ...
    cudaMemcpyToSymbol(table_capacity,
				      &capacity,
				      sizeof(unsigned int));
    ...
    cudaMemcpyToSymbol(table_values,
				      &values,
				      sizeof(float *));       
    ...
    cudaMemcpyToSymbol(table_entries, 
				      &entries,
				      sizeof(unsigned int *));
    ...
    cudaMemcpyToSymbol(table_rank,
				      &ranks,
				      sizeof(char *));

    signed short *zeros;
    allocateCudaMemory((void**)&zeros, capacity*sizeof(signed short));
    cudaMemcpyToSymbol(table_zeros,
				      &zeros,
				      sizeof(char *));
    ...
    cudaMemset((void *)keys, 0, capacity*kd*sizeof(signed short));
    cudaMemcpyToSymbol(table_keys,
				      &keys,
				      sizeof(unsigned int *));
    #endif
...
template <int vd> static void resetHashTable() {
  cudaMemset((void*)table_values, 0, table_capacity*vd*sizeof(float));
}
...
static float* swapHashTableValues(float *newValues) {
    float * oldValues;
    cudaMemcpyFromSymbol(&oldValues,
					table_values,
					sizeof(float *));
    cudaMemcpyToSymbol(table_values,
				      &newValues,
